...
export class CustomerCardService {
  constructor(@InjectModel(CustomerCard) private customercardRepository:typeof CustomerCard) { }
  
  async create(createCustomerCardDto: CreateCustomerCardDto) {
    const card =  await this.customercardRepository.create(createCustomerCardDto);
    return card
  }
  ...
  async mainCard(id:number,cardId:number):Promise<CustomerCard[]>{
    ...
    if(!customCard) throw new HttpException(
      'CustomerCard is not found',
      HttpStatus.NOT_FOUND
    )
    if(customCard.length < 2) {
      for(let x of customCard){
  ...
  async findAll() {
    return await this.customercardRepository.findAll({include:{all:true}});
  }
